...
### TASK-1 — Написати тести
**Поточний стан:** Тести для бекенду є (`test_auth.py`, `test_characters.py`, `test_sessions.py`, `test_campaigns.py`). Для фронтенду — майже відсутні (`authStore.test.js`, `tableStore.test.js` є але порожні або мінімальні).
**Що потрібно покрити:**
- Frontend: `useWebSocket` hook — підключення, обробка подій, cleanup
- Frontend: `EvidenceBoard` — рендер карток, drag, connect mode
- Frontend: WS event handlers — `card.published`, `card.moved`, `dice.rolled`
- Backend: додати тест на `publish` action з перевіркою WS broadcast
- Backend: додати тест що гравець без `SessionCharacter` не отримує dice broadcast